...
traindata.AttackAngle = [];traindata.PitchRateFiltered = [];traindata.PitchRate = [];
traindata.ElevatorAngle = [];traindata.LiftCoefficient = [];
for i = 1:numel(AOAdata_laishui)
    % 提取当前元素中的结构体字段值
    traindata.AttackAngle = [traindata.AttackAngle; AOAdata_laishui{i}.alpha];
    traindata.PitchRate = [traindata.PitchRate; AOAdata_laishui{i}.wy];
    traindata.ElevatorAngle = [traindata.ElevatorAngle; AOAdata_laishui{i}.elevator];
    traindata.LiftCoefficient = [traindata.LiftCoefficient; AOAdata_laishui{i}.Cl];
end
...
% Apply a rolling window filter to the pitch rate
